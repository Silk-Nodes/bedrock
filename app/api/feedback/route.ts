...
export async function GET(req: Request) {
  ...
  try {
    const r = await fetch(`${INDEXER_URL}/api/v1/feedback?voter=${encodeURIComponent(voterId)}&status=${encodeURIComponent(status)}&sort=${encodeURIComponent(sort)}`, { cache: "no-store" });
...
export async function POST(req: Request) {
  ...
  try {
    const r = await fetch(`${INDEXER_URL}/api/v1/feedback?voter=${encodeURIComponent(voterId)}&ip=${encodeURIComponent(ip)}`, {
